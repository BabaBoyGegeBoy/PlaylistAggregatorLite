...
// buildSongList 根据QQ音乐响应数据构建歌曲列表（同时填充结构化 SongsDetail）
func buildSongList(response *models.QQMusicResp, detailed bool) *models.SongList {
	...
	detail := make([]models.SongItem, 0, len(songList))
	...
	for _, song := range songList {
		...
		var nameUsed string
		if detailed {
			nameUsed = song.Name // 使用原始歌曲名
		} else {
			nameUsed = utils.StandardSongName(song.Name) // 去除多余符号
		}

		builder.WriteString(nameUsed)
		builder.WriteString(" - ")
